...
export default function Dashboard() {
  ...
  const todayProgress = todayTasksCount > 0 
    ? (completedTodayCount / todayTasksCount) * 100 
    : 0;
  
  const completedWeekCount = tasks?.filter(task => task.completed)?.length || 0;
  const totalTasksCount = tasks?.length || 0;
  const weekProgress = totalTasksCount > 0 
    ? (completedWeekCount / totalTasksCount) * 100 
    : 0;
  
  const upcomingDeadlinesCount = upcomingTasks?.length || 0;
  const nextDeadline = upcomingTasks && upcomingTasks.length > 0 
    ? upcomingTasks[0].title + (upcomingTasks[0].dueDate ? ` (${new Date(upcomingTasks[0].dueDate).toLocaleDateString()})` : '')
    ...
    <div>
      ...
      <Card className="mb-6">
        <CardContent className="pt-6">
          ...
          <DailyPlanner userId={userId} date={date} />
        </CardContent>
      </Card>
