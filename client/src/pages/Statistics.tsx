...
import { ChartBarStacked, CheckCircle, Clock, Calendar, ChartPie, TrendingUp } from "lucide-react";
...
export default function Statistics() {
  ...
  const getProductivityByCategory = () => {
    ...
    tasks.forEach(task => {
      if (task.completed && categories[task.category as keyof typeof categories] !== undefined) {
  ...
  const totalCategoryTasks = categoryData.reduce((sum, item) => sum + item.count, 0);
  ...
  const getMostProductiveDay = () => {
    ...
    const mostProductive = productivityData.reduce((max, day) => 
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex space-x-2">
                  ...
                  <Button 
                    variant="ghost" 
                    size="sm"
                    className={timeRange === 'year' ? 'text-primary' : 'text-gray-500'}
                    ...
                    {productivityData.map((day, index) => (
                      <div key={index} className="flex flex-col items-center flex-1">
                        <div 
                          ...
                          title={`${day.count} tasks on ${format(day.date, 'MMM dd')}`}
                        ></div>
                    ...
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="text-sm text-gray-500">
                        {totalCategoryTasks > 0 
                          ? `${Math.round((item.count / totalCategoryTasks) * 100)}%` 
                          : '0%'}
                      </span>
                    </div>
                    <Progress 
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-6">
                  ...
                  <div>
                    <div className="flex justify-between mb-1">
                      <span className="text-sm text-gray-500">On-time Completion</span>
                      <span className="text-sm font-medium">
                    ...
                    <Progress 
                      value={taskStats.completed > 0 
                        ? ((taskStats.completed - taskStats.overdue) / taskStats.completed) * 100 
                        : 0} 
                      className="h-2"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4">
                    <div className="text-center p-4 bg-gray-50 rounded-lg">
                      <h4 className="text-sm text-gray-500 mb-1">Average Completion Time</h4>
                      <p className="text-xl font-bold text-primary">{getAverageCompletionTime()}</p>
                    </div>
                    <div className="text-center p-4 bg-gray-50 rounded-lg">
                      <h4 className="text-sm text-gray-500 mb-1">Task Efficiency</h4>
